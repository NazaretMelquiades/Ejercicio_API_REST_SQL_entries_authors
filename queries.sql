...
UPDATE entries
	SET content='Back is back', date='2024-06-17', title='holaholitavecinito', category='Software'
	WHERE title='Estamos de Lunes de Back';
--Borrar entry
DELETE FROM entries
WHERE title= 'El rayo gana la champions'
-- Selecciona todos los autores 
SELECT * FROM authors;
-- Selecciona por nombre los datos del autor buscado
SELECT * FROM authors
WHERE email= 'alejandru@thebridgeschool.es'
--Actualizar datos de un autor 
UPDATE authors
SET name='Luisa', surname='Suarez', email='hola@thebridge.com' image='https://randomuser.me/api/portraits/thumb/men/75.jpg'
WHERE email='alejandru@thebridgeschool.es';
-- Borrar autor
DELETE FROM authors
WHERE email='alejandru@thebridgeschool.es'
